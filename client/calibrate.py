...
def rms(arr):
    ...
    # print('r', arr, (np.sum(np.square(arr)) / np.size(arr)) ** (1/2))
...
def chunk_and_rms_sound(full_sound, window_size=2048):
    ...
    for window_start in range(0, len(full_sound), hop_size):
        ...
        # if np.size(window) == 0:
        #     print("sz", np.size(window))

        # print("Window index:", window_start)
        # print("Window shape:", np.shape(window))
        # print("Window type:", type(window))
        # print("First few elements:", window[:3])
...
def measure_calibration():
    ...
    signal_quartiles = statistics.quantiles(signal_rmses)
    ...
    # alpha = 0.5
    # threshold = alpha * avg_signal_rms + (1 - alpha) * avg_noise_rms
    # print(f'Calibration threshold: {threshold}')
    ...
    #     input('Measuring noise: hit [ENTER] to start')
    #     time.sleep(1.0)
    #     noise_trial = record_audio(record_seconds=trial_duration)

    #     rmses = chunk_and_rms_sound(full_sound=noise_trial)
    #     print(len(rmses))
    #     print(statistics.quantiles(rmses))
    #     raise NotImplementedError


    #     # trial_rms = rms(noise_trial)
    #     # noise_rmses.append(trial_rms)
    #     print(f'Noise trial RMS: {trial_rms}')

    # avg_noise_rms = statistics.median(noise_rmses)
    # print(f'Average noise RMS: {avg_noise_rms}')

    # signal_rmses = []
    # for _ in range(signal_trials):
    #     input('Measuring signal: hit [ENTER] to start')
    #     time.sleep(1.0)
    #     signal_trial = record_audio(record_seconds=trial_duration)
    #     trial_rms = rms(signal_trial)
    #     signal_rmses.append(trial_rms)
    #     print(f'Signal trial RMS: {trial_rms}')

    # avg_signal_rms = statistics.median(signal_rmses)
    # print(f'Average signal RMS: {avg_signal_rms}')

    # alpha = 0.25
    # threshold = alpha * avg_signal_rms + (1 - alpha) * avg_noise_rms
    # print(f'Calibration threshold: {threshold}')

    # return threshold, avg_noise_rms, avg_signal_rms
...
def apply_calibration(noise_quartiles, noise_mean, noise_std, signal_quartiles, signal_mean, signal_std):
    ...
    # j['AVG_NOISE_RMS'] = avg_noise_rms
    # j['AVG_SIGNAL_RMS'] = avg_signal_rms
...
def denoise_signal(signal, noise_quartiles, signal_quartiles):
    ...
    # print(window_rmses.tolist())

    initial_mask = window_rmses >= threshold

    # print(window_rmses)
    # print("num", np.sum(initial_mask.astype(np.int16)), 'of', len(window_rmses))

    context = 1
    smoothed_mask = scipy.ndimage.maximum_filter1d(initial_mask, size=2 * context + 1)

    # print("num", np.sum(smoothed_mask.astype(np.int16)), 'of', len(smoothed_mask))
